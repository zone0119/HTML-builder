...
async function init() {
    ...
    await fs.promises.mkdir(newFolder, { recursive: true }).then(value => {
        ...
        fs.createWriteStream(mergeStylesFolder, encode).write('\n');
        
    }).then(value => { recursionCopyDirs(assetsFolder); })
        .then(value => { 
            const mergeStylesFolder = path.join(getDirName, 'project-dist', 'style.css');
            const writeStream = fs.createWriteStream(mergeStylesFolder, encode); 
            copyStyles(getSrcStylesFolder, writeStream); })
        .then(value => { recursionCopyFiles(assetsFolder); })
        .then(value => { getText(); })
        .then(value => { createObjComponent(componentFolder); });
}
...
async function copyStyles(folder, writeStream) {
